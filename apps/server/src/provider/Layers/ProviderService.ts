...
import * as McpCapabilities from "../../mcp/McpCapabilities.ts";
...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  const turnSemaphoresRef = yield* Ref.make(new Map<ThreadId, Semaphore.Semaphore>());
  const serializeTurnStart = <A, E, R>(
    threadId: ThreadId,
    effect: Effect.Effect<A, E, R>,
  ): Effect.Effect<A, E, R> =>
    Effect.flatMap(
      Ref.modify(turnSemaphoresRef, (semaphores) => {
        const existing = semaphores.get(threadId);
        if (existing !== undefined) return [existing, semaphores];
        const created = Semaphore.makeUnsafe(1);
        semaphores.set(threadId, created);
   ...
   * what disables agent toolsets everywhere: every adapter already treats a
   * missing session as "no MCP server", and the `/mcp` endpoint accepts
   * nothing but tokens issued from this path. Capability scopes are
   * independent (Browser access vs Project work), so the credential survives
   * with either family enabled.
   */
  /**
  ...
  const sendTurn: ProviderServiceMethod<"sendTurn"> = Effect.fn("sendTurn")(function* (rawInput) {
    ...
    return yield* serializeTurnStart(
      input.threadId,
      Effect.gen(function* () {
