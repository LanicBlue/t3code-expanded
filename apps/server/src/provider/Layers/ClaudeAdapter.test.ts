...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("passes the instance auto-compact window into query settings when configured", () => {
    ...
    return Effect.gen(function* () {
      ...
      // so the window must also ride the flag-settings env block, the one
      // tier that outranks the user-level settings env.
      assert.deepEqual((settings as Record<string, unknown> | undefined)?.env, {
